// Fix admin@test.com: ensure roles exist for tenant and assign Tenant Admin
const http = require('http');

function dbQuery(sql) {
  return new Promise((resolve, reject) => {
    // Use the API's internal DB connection via a node script running inside the container
    // This script runs directly against postgres
  });
}

// We'll run this as SQL via psql
const sql = `
-- 1. Find admin@test.com's tenant
DO $$
DECLARE
  v_user_id uuid;
  v_tenant_id uuid;
...
BEGIN
  ...
  -- 2. Check if roles exist for this tenant
  IF NOT EXISTS (SELECT 1 FROM roles WHERE tenant_id = v_tenant_id) THEN
    RAISE NOTICE 'No roles found for tenant, creating default roles...';
  ...
  -- 4. Assign Tenant Admin role to user
  ...
  -- 5. Check if company exists for this tenant
